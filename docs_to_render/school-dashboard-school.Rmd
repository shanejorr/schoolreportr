...
all_race_population <- census_demographics %>%
  filter(str_detect(NAME, 'Tract')) %>%
  group_by(NAME) %>%
  mutate(total_estimate = max(estimate), total_moe = max(moe)) %>%
  ungroup() %>%
  filter(variable %in% races_to_use) %>%
  mutate(
    ...
    across(c('estimate', 'moe', 'perc_moe'), ~round(., 0))
  )

race_var <- 'White'

specific_race_population <- all_race_population %>% filter(variable == !!race_var)

labels <- glue::glue(
  "<strong>{race_var} Population</strong><br/>
   <strong>{specific_race_population$estimate}</strong> {race_var} residents (+/- {specific_race_population$moe})<br/>
   <strong>{specific_race_population$perc_estimate}</strong> {race_var} population (+/- {specific_race_population$perc_moe})"
  ) %>%
