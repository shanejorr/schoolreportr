...
saipe_years <- data_years_available('saipe')
ccd_years <- data_years_available('ccd')

school_directory <- get_ccd_directory(org_level, nces_num, max(ccd_years))
...
school_enrollment <- get_ccd_enrollment(org_level, nces_num, years, grades)

district_poverty <- get_district_in_poverty(district_leaid_number, max(saipe_years))
...
state_assessment <- get_edfacts_state_assessments(org_level, state_fips_code(state_abb), years, assessment_grades) %>%
  assessment_scores_by_race(org_level, nces_num) %>%
...
cities_table <- table_cities_in_single_district(district_shapefile, state = state_abb) %>% 
  cities_in_district()
```
...
all_year_plts <- map2(subjects, plt_titles, function(.x, plt_title) {
  ...
  all_years_total %>%
    ...
    custom_hc_tooltip(
      create_html_tooltip('geography', glue('{.x}_pct_pass_clean'),
...
all_years_race_district <- map2(subjects, plt_titles, function(.x, plt_title) {
  ...
  school %>%
    ...
    custom_hc_tooltip(
      create_html_tooltip('race', glue('{.x}_pct_pass_clean'),
                                          glue('{.x}_test_num_valid'))
      )
...
all_years_race_total <- map2(subjects, plt_titles, function(.x, plt_title) {
  ...
  hc_plot_grouped_bar(all_years_race, 'race', glue('{.x}_pct_pass'), 'geography', glue("{plt_title} in grades {grade_span}"))
...
tracts_in_district <- census_tracts_in_district(state_abb, district_shapefile)
```
...
census_demographics <- acs_demographic_data(
  ...
  mutate(variable = rename_reorder_race_education(variable)) %>%
  arrange(variable)

hc_plot_grouped_bar(census_demographics, 'variable', 'perc_demo', 'geography', "Racial breakdown of community")
```
...
hc_plot_grouped_bar(census_education, 'variable', 'perc_demo', 'geography', "Educational attainment of community")
```
